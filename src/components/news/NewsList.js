...
class NewsList extends React.Component {
  ...
  fetchNews = (concat) => {
    ...
    Axios.get(
      `https://api.nytimes.com/svc/search/v2/articlesearch.json?api-key=OAD0Qz0csaoDZLpw5ZR74TCeSjynnabJ&sort=newest&page=${this.state.page}`
    ).then(
      (res) => {
        this.setState({
          data: concat
            ? this.state.data.concat(res.data.response.docs)
...
const mapStateToProps = (state) => {
  return {
    foods: state.newsReducer.newsList,
  };
};
